...
| **Logging** | Zap | Uber에서 만든 고성능 로깅 라이브러리 |
...
- **변수/함수명**: 명확하고 직관적인 영어 이름 사용 (`getUser`, `savePost`).
- **에러 처리**: Go 스타일의 에러 반환 (`if err != nil`) 준수, 래핑하여 문맥 제공.
- **구조화**: 기능 추가 시 `internal/feature` 내에 새로운 패키지 생성 원칙.
